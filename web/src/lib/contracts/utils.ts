 ...
 */
export function parseProposalFromContract(contractData: any, proposalId?: bigint): Proposal {
  return {
    id: proposalId || BigInt(contractData.id),
 ...
 */
export function isProposalActive(proposal: Proposal, currentTimestamp?: bigint): boolean {
  const now = currentTimestamp || BigInt(Math.floor(Date.now() / 1000));
  return (
    proposal.status === ProposalStatus.Active && now < proposal.votingDeadline
  );
}
